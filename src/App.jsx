    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <section className="py-12 bg-gray-50 border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6">
          <p className="text-center text-sm font-bold text-gray-400 uppercase tracking-widest mb-8">Expertise in World-Class Systems</p>
          <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12 opacity-70 grayscale hover:grayscale-0 transition-all">
             {partners.map((item) => (
               <span key={item} className="text-lg font-bold text-gray-600 border border-gray-200 px-4 py-2 rounded-lg bg-white shadow-sm">
                 {item}
               </span>
             ))}
          </div>
      ...
      {/* Currency Switcher */}
      <div className="flex justify-center items-center gap-3 mt-12 px-6 text-center">
        <span className="text-sm font-medium uppercase tracking-wide" style={{ color: brandColor }}>Choose your currency:</span>
        <select 
          value={currency} 
          onChange={(e) => setCurrency(e.target.value)} 
          className="p-3 bg-white border border-gray-200 rounded-lg shadow-sm focus:ring-2 outline-none cursor-pointer"
        >
      ...
      <section className="py-16 px-6 bg-white mt-12">
        <h2 className="text-3xl font-bold text-center mb-12">International Pricing</h2>
        <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {["Starter", "Growth"].map((plan, i) => (
            <div key={i} className="bg-gray-50 border border-gray-200 rounded-3xl p-10 text-center hover:shadow-xl transition-all duration-300">
      ...
      <section className="py-20 px-6 text-center text-white" style={{ backgroundColor: brandColor }}>
        <h2 className="text-4xl font-bold mb-4">Let’s Clean Up Your Books</h2>
        <p className="mb-10 text-white text-opacity-80 text-lg">Whether you are in KZN or London, we have you covered.</p>
        <a href={WHATSAPP_LINK} className="inline-flex items-center gap-2 bg-white rounded-full px-10 py-4 text-xl font-bold hover:bg-gray-100 transition shadow-lg" style={{ color: brandColor }}>
          <CalendarIcon /> Schedule Discovery Call
        </a>
      </section>
